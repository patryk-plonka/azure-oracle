...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "data trapped/scattered across sources + late-discovery decision paralysis"
    - topic: "primary persona"
      decision: "AI agent generating/reviewing IaC via MCP; human in the loop"
    - topic: "insight"
      decision: "limits spread across docs, GitHub issues, forums — no single source-backed dataset"
    - topic: "role model"
      decision: "flat user model; ingestion/source-registry operator-side out-of-band"
    - topic: "license"
      decision: "Demo license only for MVP, checked per protected request"
    - topic: "mvp scope"
      decision: "scope down: seed from curated CSV, one query core (MCP + 1 REST endpoint), keep full OAuth, defer ingestion/LLM/optional to v2"
    - topic: "timeline"
      decision: "~3 weeks after-hours, no deadline"
  frs_drafted: 17
...
# Shape Notes — AzLimits (Azure Oracle)

> Seed: `concept.md`. This file anticipates the 10 greenfield PRD sections in schema order.
> Shaping complete (phases 1–7). Ready for `/10x-prd`.
...
- An authenticated, Demo-licensed agent queries AzLimits (via the MCP tool or the
  ...
  confidence, and status, drawn from a seeded dataset of ≥ 100 verified records.
...
- v1 target: ~3 weeks of after-hours work, achieved via the scope-down below.
- Effort mode: after-hours only. No hard deadline.

### v1 scope-down (accepted)
- **Cut live ingestion for v1** — seed the ≥ 100 records from the existing curated
  `concept/azure_limitations_db.csv` via a manual import. Automated fetch / scrape
  and scheduled refresh move to v2.
- **One query core** — a single shared query engine exposed as (a) an MCP tool and
  (b) one REST search endpoint. Rich REST surface (sorting, grouping, multi-facet
  filtering) moves to v2.
- **Defer LLM-assisted extraction to v2** — curated seed data needs no extraction.
- **All optional functionality out of v1** — web UI, CLI, GitHub Action, VS Code /
  Copilot integration, JSONL export.
- **Retained in v1** — full GitHub OAuth + EULA acceptance + Demo license + API
  token generation / hashing / expiration / revocation, and per-request token +
  license validation.
...
### Data & Provenance
- FR-011: Operator can import a curated dataset of ≥ 100 verified limitation records into the query database. Priority: must-have
  > Socrates: Counter-argument considered: "100 records is too thin / the CSV may be stale." Resolution: stands — 100 verified, source-backed records beat a larger unverified set for a demo; staleness is tracked via each record's verification metadata.
...
## Forward: v2 (not part of PRD)

Deferred out of v1 by the Phase 3 scope-down — captured here so the next chain
step can pick them up; NOT part of the PRD schema:
- Automated/scheduled source ingestion (fetch, scrape, dedupe, refresh metrics).
- LLM-assisted extraction with preserved provenance/confidence/verification state.
- Rich REST query surface: sorting, grouping, multi-facet filtering, severity.
- Region/SKU-precise filtering promoted to must-have (see Open Questions #3).
- Optional surfaces: web UI, CLI, GitHub Action, VS Code / Copilot integration,
  JSONL export.
